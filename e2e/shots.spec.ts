...
test('Screenshots: Coach und Profil (hell/dunkel)', async ({ page }) => {
  ...
  // Wochencheck und Muskelkarte (Schritt 21)
  await page.getByRole('heading', { name: /Muskelkarte/ }).scrollIntoViewIfNeeded()
  await page.locator('.bm-muscle[data-status="unter"] > *').first().click()
  await page.screenshot({ path: `${OUT}/75-bodymap-dark.png` })
  await page.emulateMedia({ colorScheme: 'light' })
  await page.screenshot({ path: `${OUT}/74-bodymap-light.png` })
  await page.emulateMedia({ colorScheme: 'dark' })
  await tab(page, 'Mehr').click()

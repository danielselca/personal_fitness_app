...
- Übungen hinzufügen (Suche, Mehrfachauswahl), entfernen (Karte → Bearbeiten oder Sortiermodus), im Sortiermodus mit Pfeilen umsortieren. Notizfeld je Übung (eingeklappt). **Neue Übung direkt aus der Auswahl anlegen:** Findet die Suche nichts, bietet die Liste „‚<Suchtext>‘ als neue Übung anlegen“ an; die Übung wird sofort ins Training übernommen, ohne den Bildschirm zu verlassen. Details (Gerätenummer, Pause, Schrittweite) können später im Katalog ergänzt werden.
...
**Programme (Schema 9, Schritt 18):** Ein Programm ist immer die Kopie des Nutzers; jeder Tag verweist auf eine eigene Vorlage mit `programId` (erscheint nicht in der Vorlagen-Liste). Beim Übernehmen eines mitgelieferten Programms (`src/programs/builtin.ts`) werden zuerst vorhandene Übungen genutzt (verknüpft → `ex-lib-<id>` → gleicher Name), nur fehlende aus der Bibliothek übernommen. „Nächstes Training“ wird aus dem zuletzt abgeschlossenen Training des Programms abgeleitet (Tag danach), gespeichert ist nur `settings.activeProgramId`. Beim Start kopiert das Training Satzzahl, Wdh.-Bereich und Pause aus der Vorlage; der Pausentimer nimmt die Vorlagen-Pause vor der Übungspause. Duplizieren legt neue Programm- und Vorlagen-IDs mit denselben Übungen an. Migration 8 → 9 hebt nur die Version; neue Listen beginnen leer, `weeklyGoal` = 3. **Coach (Schema 10, Schritt 20):** `WorkoutEntry.rating` (leicht/passend/schwer) und `WorkoutEntry.coach` ({kind, note}) halten Bewertung und den Vorschlag fest, mit dem ein Eintrag vorbelegt wurde; `bodyLog[]` {id, date, weightKg}; `settings.coachProgression` (Standard an) und `settings.profile` {goal, experience}. Steigerung nur bei Vorlagen-Einträgen mit Wdh.-Bereich (doppelte Progression, siehe `src/domain/coach/progression.ts`), nie für Physio. „Schonen beenden“ setzt `until` auf gestern statt zu löschen, damit der Wiedereinstieg (−10 %) erkannt wird. **Coach II (Schritt 21, kein neues Schema):** Der Wochencheck wertet nur vorhandene Daten aus (`src/domain/coach/{week,volume,frequency,balance,rest,deload,hints}.ts`): abgehakte Sätze von Kraftübungen mit Hauptmuskeln der ISO-Woche; Kategorie Mobilität/Kardio/Physio zählt nicht, Übungen ohne Muskelzuordnung werden gemeldet. Pausen = Abstand der `doneAt` aufeinanderfolgender Sätze derselben Übung (≤ 10 min, ohne Halteübungen). Vorschläge ändern nichts automatisch; Sprünge in andere Tabs laufen über `navStore.target`. **Mit Claude besprechen (Schritt 22, kein neues Schema):** `buildBrief` (`src/domain/briefing.ts`) erzeugt einen deutschen Text ≤ 6 000 Zeichen; kein Netzwerkzugriff, der Text verlässt die App nur per Teilen/Kopieren. Programmvorschläge kommen als JSON `{ "format": "fitness-app-programm/v1", name, sessionsPerWeek?, goal?, days: [{ name, exercises: [{ id? | library? , name, sets, reps: n | [von, bis], holdSec?, restSec?, note? }] }] }` (Grenzen: 1–7 Tage, 1–15 Übungen, Sätze 1–10, Wdh. 1–50, Halten 5–600 s, Pause 0–600 s); `importProgram` legt ein normales eigenes Programm an (`copiedFrom: 'claude'`), vorhandene Übungen werden weiterverwendet, unbekannte als eigene Übung neu angelegt.

**Grafiken (19a):** `public/media/v1/ex/<bibliotheks-id>-<n>.svg` (Workout Guide 3 Phasen, Everkinetic 2), CC BY-SA 4.0 mit `ATTRIBUTION.json` je Datei; nicht im Precache, sondern Laufzeit-Cache „media-v1“ (CacheFirst, 800 Dateien) plus stilles Vorladen der verknüpften Übungen. Anzeige als gestapelte Bilder mit CSS-Überblendung, Figur über `--media-filter` passend zu Hell/Dunkel.

**Schonen (18b):** `restrictions[]` gelten bis einschließlich `until` (ohne Datum unbefristet). Eine Übung ist betroffen, wenn ihre belasteten Bereiche (`Exercise.loads` vor Bibliothek) oder Hauptmuskeln geschont sind; sie wird im Training markiert, „Übung tauschen“ schlägt nur unbelastete Übungen vor (gleiches Muster oder gleiche Muskeln, deine Übungen zuerst). Tauschen im Training behält Position, Satzzahl und Zielbereich, nicht bei bereits abgehakten Sätzen; in der Vorlage bleiben Sätze und Bereich, die Pause der alten Übung entfällt.

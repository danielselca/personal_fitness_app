...
| 20 Coach I (Roadmap) | erledigt | Schema 10 (`WorkoutEntry.rating/coach`, `bodyLog`, `settings.coachProgression/profile`); Steigerungsvorschläge nach doppelter Progression für Übungen mit Zielbereich (`src/domain/coach/progression.ts`: Gewicht +1 Schritt bei allen Sätzen am oberen Ende, sonst +1 Wdh.; ohne Gewicht Wdh. → Satz → Variante; Halten +5 s; „schwer“/„leicht“; −10 % nach ≥ 14 Tagen oder Schonung; geschont keine Steigerung; Physio nie) mit Hinweis und „Wie letztes Mal“ auf der Karte; „Wie war’s?“ im Abschluss; Bestwerte (`records.ts`, Epley ≤ 10 Wdh.) in der Zusammenfassung und im Übungsdetail; Coach-Tab (5 Tabs, nachgeladen) mit Wochenring, Serie, nächstem Programm-Tag, Fortschritt je Übung (Faustregel „steht seit 3 Einheiten“), letzten Bestwerten, Schonung; Profil mit Ziel, Erfahrung, Körpergewicht-Verlauf; Coach abschaltbar; 221 Unit-, 21 E2E-Tests |
| 21 Coach II (Roadmap) | erledigt | Wochencheck im Coach-Tab (diese/letzte Woche, KW) mit vier Ampeln und Gesamturteil: Regelmäßigkeit, Fortschritt, Volumen & Balance, Dichte; harte Sätze je Muskelgruppe (`src/domain/coach/volume.ts`, Hauptmuskel 1, mitbeteiligt ½, Ziel nach Profil 10–20 bzw. 6–12), Häufigkeit großer Gruppen (`frequency.ts`, ≥ 2×), Drücken : Ziehen und Ober- : Unterkörper (`balance.ts`), echte Pausen aus `doneAt` (`rest.ts`), leichtere Woche als Faustregel (`deload.ts`); bis zu 3 begründete Vorschläge (`hints.ts`), ein Tipp öffnet Programm-Tag oder Übung (`navStore.target`); geschonte Bereiche werden nicht bewertet; Muskelkarte vorne/hinten (`BodyMap.tsx`) mit Legende, Antippen und Liste; kein neues Schema; 241 Unit-, 22 E2E-Tests |
| 22 Mit Claude besprechen (Roadmap) | erledigt | Coach-Tab → „Mit Claude besprechen“: Brief (`src/domain/briefing.ts`, ≤ 6 000 Zeichen, kürzt schrittweise) mit Frage, Profil, aktivem Programm samt Übungs-IDs, geschonten Bereichen, 8 Wochen Kennzahlen, Übungen (Bestwert, letzte Einheit, Trend), Befunden des Wochenchecks und Antwortformat; ohne Namen und Trainingsnotizen, Körpergewicht nur auf Wunsch; Teilen (iOS-Menü → Claude-App) oder Kopieren mit Rückfall-Textfeld. Programm-Import (`src/domain/programJson.ts`): JSON-Block `fitness-app-programm/v1` in der eingefügten Antwort finden (letzter gewinnt, typografische Anführungszeichen), prüfen mit deutschen Fehlermeldungen, Übungen zuordnen (ID → Bibliothek → Name/Alias → Bibliotheksname/Englisch → neu), Vorschau mit Kennzeichen und ⚠ bei Schonung, als eigenes Programm anlegen (gemeinsamer Kern `createProgramData`), wahlweise aktiv; Einstieg auch unter Training → Programme; Blätter nachgeladen; kein neues Schema; 258 Unit-, 24 E2E-Tests |

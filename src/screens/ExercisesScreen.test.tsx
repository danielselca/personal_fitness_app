...
describe('Bibliothek im Übungen-Tab (Schritt 17)', () => {
  ...
  it('Formular: eigene Ausrüstung, Kategorie und Muskeln', () => {
    ...
    fireEvent.click(within(within(dialog).getByRole('group', { name: 'Belastete Bereiche' })).getByRole('button', { name: 'Schulter' }))
    fireEvent.click(within(dialog).getByRole('button', { name: 'Speichern' }))
    expect(ex('ex-adduktion')).toMatchObject({ equipment: 'seilzug', category: 'physio', muscles: { primary: ['brust'], secondary: ['schulter-vorne'] }, loads: ['schulter'] })

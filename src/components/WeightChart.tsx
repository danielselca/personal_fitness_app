...
export function WeightChart({ points }: { points: WeightPoint[] }) {
  const data = points.filter((p) => p.maxWeightKg !== null) as (WeightPoint & { maxWeightKg: number })[]
  if (data.length === 0) {
  ...
  const ys = shown.map((p) => p.maxWeightKg)
  let min = Math.min(...ys)
  let max = Math.max(...ys)
  if (min === max) {
    min -= 2.5
  ...
  const path = shown.map((p, i) => `${i === 0 ? 'M' : 'L'}${x(i).toFixed(1)},${y(p.maxWeightKg).toFixed(1)}`).join(' ')
  const ticks = [min + span * 0.1, (min + max) / 2, max - span * 0.1]

  return (
    <figure className="chart" aria-label="Gewichtsverlauf">
      <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Gewichtsverlauf" width="100%" height="auto">
        {ticks.map((t, i) => (
          <g key={i}>
            <line x1={padL} x2={W - padR} y1={y(t)} y2={y(t)} className="chart-grid" />
            <text x={padL - 6} y={y(t) + 4} textAnchor="end" className="chart-tick">{formatNumber(Math.round(t * 2) / 2)}</text>
          </g>
        ))}
        <path d={path} className="chart-line" />
        {shown.map((p, i) => (
          <g key={p.workoutId}>
            <circle cx={x(i)} cy={y(p.maxWeightKg)} r={4} className="chart-dot" />
            <text x={x(i)} y={y(p.maxWeightKg) - 9} textAnchor="middle" className="chart-label">{p.reps}×</text>
          </g>
        ))}
        {shown.length > 0 && (
          <>
      ...
      <figcaption className="muted" style={{ fontSize: 13 }}>
        Höchstgewicht je Training, Beschriftung = Wiederholungen{data.length > 20 ? ` (letzte 20 von ${data.length})` : ''}
      </figcaption>
    </figure>
  )
}
